...
public class UtilisateurService {
	...
	public BaseModelPagination getDetails(BaseModel model, int page) throws Exception {
		BaseModelPagination pagination = null;
		switch(model.instance()){
			case "Profil" : pagination = new BaseModelPagination(UtilisateurView.class, 10, page);
							getService().findAllByBaseModel(pagination, model);
							break;
			case "Groupe" : pagination = findAllByBaseModel(model, AgentView.class, page);break;
			case "Agent"  : pagination = findAllByBaseModel(model, GroupeView.class, page);break;
			case "Utilisateur" : pagination = findAllByBaseModel(((Utilisateur)model).getProfil(), GroupeView.class, page);break;
		}
		return pagination;
	}
	...
	public BaseModelPagination findAllByBaseModel(BaseModel model, Class<? extends BaseModel> classe, int page) throws Exception{
		BaseModelPagination pagination = new BaseModelPagination(classe, 10, page);
		this.getService().findAllByBaseModel(pagination, model, "liste".concat(model.instance()));
		return pagination;
	}
